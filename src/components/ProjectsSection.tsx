...
const ProjectsSection: React.FC = () => {
  ...
  return (
    <section id="projects" ref={sectionRef} className="py-20 overflow-hidden">
      ...
      <div 
        ref={containerRef}
        className="flex gap-8 px-6 md:px-12 pb-8 md:pb-0"
        style={{ width: 'max-content' }}
      >
        {projects.map((project, index) => (
          <div
            key={project.id}
            className="project-card glass-card flex-shrink-0 w-80 md:w-96 group cursor-pointer
          ...
          >
            ...
            <div className="relative overflow-hidden rounded-t-2xl h-48 md:h-56">
              ...
              <div className="absolute inset-0 flex items-center justify-center gap-4 opacity-0 group-hover:opacity-100 transition-all duration-300">
                <a 
                  href={project.github}
                  className="btn-glow p-3 rounded-full transform -translate-y-4 group-hover:translate-y-0 transition-transform duration-300"
                >
                ...
                {project.tech.map((tech) => (
                  <span 
                    ...
                    className="px-3 py-1 text-xs font-medium bg-neon-blue/20 text-neon-blue rounded-full
                               border border-neon-blue/30 hover:bg-neon-blue/30 transition-colors duration-300"
                  >
                    {tech}
                  </span>
                ))}
